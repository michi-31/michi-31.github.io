...
table2_new <- table2 |>
  pivot_wider(
    names_from = type, # type列から名前を取得する
    values_from = count # count列から値を取得する
  ) |>
  mutate( # 割合を計算して新しい列（rate）を作成する
...
- separate(): 特定の区切り文字で列を分割し、複数の列に展開する関数 
- convert = TRUE: 分割後の列が可能な限り自動的に数値型に変換されるオプション
```{r}
table3_new <- table3 |>
  separate(
    rate, # 列名を指定する
    into = c("cases", "population"), # 新しい列を作成する
    sep = "/", # スラッシュで分割する
    convert = TRUE # 文字列を数値に変換する
  ) |>
  mutate( # 割合を計算して新しい列（rate）を作成する
    rate = (cases / population ) * 10000
  )
