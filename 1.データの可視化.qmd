...
# ggplot2を用いたグラフ作成
## 必要なパッケージをロード
- tidyverse: ggplot2を含むRの基本機能を提供するパッケージ
- palmerpenguins: 南極のパーマーランドの３つの島に生息するペンギンの身体の測定値に関するデータ（penguins）を提供するパッケージ
- ggthems: 色覚以上に配慮したカラーパレットを提供するパッケージ
```{r}
library(tidyverse)
...
## penguinsデータフレームの確認
- glimpse(): 全ての変数と各変数の最初のいくつかの観測地を表示する
- View(): インタラクティブなデータビューアを表示する
```{r}
glimpse(penguins)
#View(penguins)
```
...
## 1.散布図の作成
- mapping引数: プロットの可視化特性（aesthetic）を定義する。aes()関数で軸などのを設定する。
- geom_: プロットが使用する幾何オブジェクトを定義する。
  - geom_point(): 散布図
  - geom_bar(): 棒グラフ
  - geom_line(): 折れ線グラフ
  - geom_boxplot(): 箱ひげ図
...
ggplot(
  data = penguins,
  mapping = aes(x = flipper_length_mm, # x軸の設定
                y = body_mass_g, # y軸の設定
...
## 2.散布図に直線を表示
### ペンギンの種類ごとに直線を引く
- geom_smooth(method = "lm"): linear-modelにフィットした線を描画するレイアを追加する。
```{r}
ggplot(
  data = penguins,
  mapping = aes(x = flipper_length_mm, # x軸の設定
...
## 3.ラベルの改善
- colorに加えてshapeをエステティックにマッピングして、ペンギンの種類ごと異なる形を使用する。
- labs(): プロットラベルにタイトルなどを指定する。
- scale_color_colorblind(): カラーパレットを色覚以上に対応した改善を行う。
```{r}
ggplot(
  data = penguins,
